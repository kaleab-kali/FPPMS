		...
		(emp: EmployeeWithSuperior) => (
			<div key={emp.id} className="flex items-center gap-4 p-3 border rounded-lg hover:bg-muted/50 transition-colors">
				...
				<div className="hidden md:block flex-1 min-w-0">
					...
					<p className="text-xs text-muted-foreground">
						{emp.department ? (isAmharic && emp.department.nameAm ? emp.department.nameAm : emp.department.name) : "-"}
					</p>
				</div>
				<div className="flex-1 min-w-0">
					{emp.directSuperior ? (
				...
				<div className="flex gap-1">
					...
					{emp.directSuperior && (
						<Button size="sm" variant="ghost" onClick={() => handleOpenRemoveDialog(emp)} title={t("superior.remove")}>
