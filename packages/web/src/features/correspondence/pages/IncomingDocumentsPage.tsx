...
import { useIncomingDocuments } from "#web/api/correspondence/correspondence.queries";
...
const IncomingDocumentsPageComponent = (): React.ReactElement => {
	...
	const { data, isLoading } = useIncomingDocuments(filter);
		...
		<div className="container mx-auto py-6 space-y-6">
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-3">
					...
					<div>
						<h1 className="text-2xl font-bold">{t("correspondence:incoming.title")}</h1>
						<p className="text-muted-foreground">{t("correspondence:incoming.description")}</p>
					</div>
				</div>
				<Button onClick={handleCreate}>
					<Plus className="h-4 w-4 mr-2" />
					{t("correspondence:actions.registerIncoming")}
				</Button>
			</div>
...
IncomingDocumentsPageComponent.displayName = "IncomingDocumentsPage";

export const IncomingDocumentsPage = React.memo(IncomingDocumentsPageComponent);
