...
export class EmployeeIdGeneratorService {
	...
	async generateEmployeeId(tenantId: string): Promise<string> {
		...
		const nextNumber = await this.getAndIncrementCounter(tenantId, currentYear);

		const paddedNumber = String(nextNumber).padStart(EMPLOYEE_ID_PADDING, "0");

		return `${EMPLOYEE_ID_PREFIX}-${paddedNumber}/${yearSuffix}`;
	}

	private async getAndIncrementCounter(tenantId: string, year: number): Promise<number> {
		const counterKey = `employeeIdCounter_${year}`;
